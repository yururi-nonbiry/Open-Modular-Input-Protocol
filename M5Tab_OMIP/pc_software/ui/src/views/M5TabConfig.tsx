...
export function M5TabConfig() {
    ...
    const syncPageIcons = useCallback(
        async (targetPage: number, override?: CellConfig[]) => {
            if (!hasIpc || !isConnected || !window.ipcRenderer) return;
    ...
    }, [hasIpc, isConnected, page, syncPageIcons]);
    ...
    const throttledPrevPage = useCallback(() => {
        const now = Date.now();
        if (now - lastPageNavTimeRef.current.prev < 250) return;
    ...
    const throttledNextPage = useCallback(() => {
        const now = Date.now();
        if (now - lastPageNavTimeRef.current.next < 250) return;
    ...
    }, [hasIpc, throttledNextPage, throttledPrevPage, fetchPorts, fetchConfig]);
    ...
    return (
        <Box sx={{ display: 'flex', flexDirection: 'column', height: '100%', flexGrow: 1 }}>
            ...
            <Box sx={{ p: 2, display: 'flex', alignItems: 'center', borderBottom: 1, borderColor: 'divider', bgcolor: 'background.paper' }}>
                <FormControl size="small" sx={{ mr: 2, minWidth: 200 }} disabled={isConnected}>
                    ...
                    <Select
                        ...
                        displayEmpty
                        renderValue={(value) => {
                            if (!value) return ports.length === 0 ? 'No ports available' : 'Select port';
                            return value;
                        }}
                ...
                <Button variant="outlined" size="small" sx={{ mr: 2 }} disabled={isConnected} onClick={fetchPorts}>
                    <RefreshIcon />
                </Button>
            ...
            <Box sx={{ flexGrow: 1, p: 2, overflowY: 'auto' }}>
                <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 2 }}>
                    {currentGridConfig.map((cell, index) => (
                        <Box key={index} sx={{ width: 'calc(16.666% - 16px)', minWidth: 80 }}>
                            <GridCell
                                config={cell}
            ...
            {/* Footer Navigation */}
            <Box sx={{ p: 1, bgcolor: 'background.paper', borderTop: 1, borderColor: 'divider', display: 'flex', justifyContent: 'center', alignItems: 'center', mt: 'auto' }}>
